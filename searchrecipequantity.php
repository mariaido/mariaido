...
<?php
	...
	if(!$seldb) //αν αποτύχει η σύνδεση μήνυμα σφάλματος
	{
	...
	while($row=mysql_fetch_array($all_rows)) 
	{
		...
		//οι παρακάτω 8 γραμμές κώδικα ελέγχουν πόσα υλικά βάλαμε στην αναζήτηση($b) και πόσα έχει η συνταγή($c)
		$a=array_intersect($id, $iddb); //αποθηκεύει τα κοινά στοιχεία των $id, $iddb στο $a
		$containsSearch = count(array_intersect($id, $iddb)) == count($id); //1 αν ο αριθμός κοινών στοιχείων == count($id) αλλιώς 0 
		$tmp = array_count_values($id); //επιστρέφει την συχνότητα με την οποία εμφανίζονται τα υλικά
		$cnt = $tmp[0]; //παίρνουμε το υλικό που εμφανίζεται πιο συχνά
		$b=count($id)-$cnt;
		$tmp = array_count_values($iddb); //επιστρέφει την συχνότητα με την οποία εμφανίζονται τα υλικά στην βάση
		$cnt = $tmp[0];
		$c= 15-$cnt; //γνωρίζουμε ότι τα max υλικά είναι 15 σε μία συνταγή
		echo ("<div id='index'>");
		
		if($containsSearch && ($b == $c)) //αν υπάρχουν στην αναζήτηση και είναι ίσα 
		{
			$key=1;
			for($z=0;$z<$i;$z++)
			{
				if($id[$z]==$iddb[$z] && $quantity[$z]<$quantitydb[$z]) //ελέγχει αν υπάρχει διαθέσιμη ποσότητα
				{
					$key=0;
				}
			}
			if($key==1) //αν τα παραπάνω ισχύουν εκτυπώνει τα ονόματα των συνταγών
			{
